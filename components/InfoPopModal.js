...
export default function InfoModal({ modalVisible, setModalVisible }) {
  return (
    ...
    >
      <div onClick={(e) => e.stopPropagation()} className="relative border-2 rounded bg-slate-300 border-slate-500">
        <button onClick={() => setModalVisible(!modalVisible)} className="absolute right-0 p-2" type="button">
          {close}
        </button>
        <div className="max-w-2xl px-12 my-10 prose-sm prose md:px-20 md:prose-base prose-li:list-disc prose-ul:list-disc marker:text-slate-700">
